...
export default function OrderSummary({ product, cartTotals, delivery, checkoutMode }) {
    const { openCouponModal, isBeansApplied, toggleBeans, beansBalance, applyCoupon, removeCoupon, appliedCoupon, coinConfig } = useCart();
        ...
        <div className={styles.Right}>
                ...
                <div className={`${styles.RewardsSection} ${checkoutMode === "subscription" ? styles.SubRewardsSection : ""}`}>
                    <label className={styles.CheckboxContainer}>
                        ...
                        <div className={styles.RewardsInfo}>
                            ...
                            <p className={styles.RewardsBalance}>Total WM Beans: {beansBalance}</p>
                        </div>
                    </label>
                    {beansBalance > 0 && (
                        <p className={styles.PotentialSavings}><span>Save up to 20% of this Order</span></p>
                    )}
                </div>
            </div>
            ...
            <div className={styles.RightThree}>
                ...
                {cartTotals.beansDiscount > 0 && (
                    <div className={styles.Subtotal}>
                        <p>Beans Discount</p>
                        <h5 style={{ color: "green" }}>- AED {cartTotals.beansDiscount.toFixed(2)}</h5>
                    </div>
                )}
                ...
                <div className={styles.EarningBadge}>
                    <p>You're earning {Math.floor(cartTotals.total * (coinConfig.pointsEarn / 100))} WM Beans on this order</p>
                </div>
            </div>
        </div>
    );
